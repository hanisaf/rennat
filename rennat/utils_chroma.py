...
def load_store(save_dir : str, openai_embeddings=True) -> VectorStore:
    ...
    embeddings = OpenAIEmbeddings() if openai_embeddings else HuggingFaceHubEmbeddings()
    index = Chroma(embedding_function=embeddings, persist_directory=save_dir)
    return index

def update_store(index: Chroma, docs: List[Document]) -> bool:
    # avoid re-embedding existing documents
    existing_docs = get_doc_names(index)
...
def save_store(index: Chroma, save_dir: str = None):
    """Saves a index to a save directory"""
    if save_dir:
        index.persist_directory = save_dir
    index.persist()
...
def get_doc_names(index: Chroma) -> List[str]:
    """Gets a list of document names."""
    docstore = index.get()
    existing_docs = {d['name'] for d in docstore['metadatas']}
    return existing_docs

def search_docs(index: Chroma, query: str, k:int = 5, meta_names : List[str] = None, exclude_names : List[str] = None, min_length=0) -> List[Document]:
